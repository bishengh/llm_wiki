...
1. **安装依赖**：在 `src` 目录中运行 `npm install`
2. **启动服务器**：在 `src` 目录中运行 `npm start`
3. **访问系统**：打开浏览器访问 `http://localhost:3000`
4. **上传文件**：使用"上传文件"功能上传文档
5. **浏览页面**：使用"查看页面"功能浏览wiki内容
6. **查询交互**：使用"查询交互"功能进行自然语言查询
7. **系统维护**：使用"系统维护"功能进行备份和维护
